    ...
    const handleShare = async () => {
        try {
            await navigator.share({
                title: 'LOLLY | Agence de Communication & Formation',
                text: 'Découvrez LOLLY, l\'agence qui propulse les marques vers l\'excellence.',
                url: 'https://lolly.sn',
            });
        } catch (err) {
            ...
            <AnimatePresence>
                {canShare && (
                    <motion.button
                        ...
                        whileTap={{ scale: 0.9 }}
                        onClick={handleShare}
                        className="w-12 h-12 bg-white/5 backdrop-blur-md border border-white/10 text-white rounded-full flex items-center justify-center shadow-xl mb-1"
