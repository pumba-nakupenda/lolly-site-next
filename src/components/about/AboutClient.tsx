...
const IconMap: { [key: string]: any } = {
    CheckCircle, Award, Users, Lightbulb, Target, Rocket
};
...
const AboutClient = () => {
    ...
    const defaultOdyssey = [
        { title: "L'Origine", content: "Née d'une vision audacieuse en plein cœur de Dakar, LOLLY est l'histoire d'un engagement : transformer des idées en messages puissants. Fondée par Amadou Mbaye GUEYE, nous portons la conviction que la communication africaine mérite une excellence sans compromis.", color: "primary" },
        { title: "L'Évolution", content: "Aujourd'hui, nous nous positionnons comme une pure agence de conseil. Nous structurons la communication des entreprises pour en faire un levier de croissance durable, soutenue par nos pôles de production et de formation.", color: "accent" }
    ];
    ...
    const defaultEcosystem = [
        { title: "Conseil Stratégique High-End", desc: "Nous auditons vos structures de communication pour y injecter de l'intelligence et de la performance durable.", icon: "Target", tags: ["Communication de Crise", "Branding Architecture", "Digital Transformation"], highlight: true },
    ...
    const title = "Plus qu'une agence, partenaire d'exception.";
    const subtitle = "Nous croyons au pouvoir des idées qui bousculent le statu quo et aux images qui impriment une vision durable.";
    const odyssey = defaultOdyssey;
    const odysseyImage = "/assets/notre-odyssee.webp";
    const values = defaultValues;
    const valuesTitle = "Notre Croyance";
    const valuesSubtitle = "Plus qu'un slogan, c'est notre boussole. Nous refusons le bruit inutile pour ne créer que du sens à haute intensité.";
    const founder = {
        name: "Amadou Mbaye GUEYE",
        role: "Founder & Chief Visionary",
        quote: "Notre héritage sera l'autonomie des talents que nous formons.",
        bio: "Amadou Mbaye GUEYE incarne l'alliance rare entre une créativité organique et une maîtrise technique pointue en automatisation et IA.",
        image: "/assets/amadou-mbaye-gueye.webp"
    };
    const ecosystem = defaultEcosystem;
                ...
                >
                    ...
                    <h1 className="text-3xl md:text-8xl font-serif font-bold text-white mb-8 md:mb-10 tracking-tight leading-[1.1]">
                        {(() => {
                            ...
                            if (lines.length > 1) {
                                return <>{lines[0]}, <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-white to-primary pb-1 px-2">{lines[1]}</span></>;
                            }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-center">
                    ...
                    >
                        ...
                        <div className="space-y-8 relative">
                            ...
                            {odyssey.map((step: any, i: number) => (
                                <motion.div
                                    key={i}
                                    whileHover={{ x: 10 }}
                                    className="pl-8 md:pl-10 relative group"
                                >
                                    <div className={`absolute left-[-2px] top-2 w-1 h-8 ${step.color === 'accent' ? 'bg-accent' : 'bg-primary'} shadow-[0_0_15px_rgba(255,215,0,0.5)] transition-all group-hover:h-full`} />
                    ...
                    >
                        ...
                        <div className="relative rounded-[2.5rem] md:rounded-[3rem] overflow-hidden border border-white/10 shadow-3xl bg-surface/20 backdrop-blur-sm p-3 md:p-4 h-[300px] md:h-[600px]">
                            <OptimizedImage
                                src={odysseyImage}
                                alt="L'Odyssée LOLLY"
                                className="rounded-[2rem] md:rounded-[2.5rem] grayscale hover:grayscale-0 transition-all duration-1000 object-cover"
                            />
                            ...
                            >
                                ...
                                <div className="w-14 h-14 md:w-16 md:h-16 bg-white/5 rounded-2xl flex items-center justify-center mb-6 md:mb-8 border border-white/10 group-hover:bg-primary/20 transition-all shadow-lg group-hover:rotate-12">
                                    {(() => {
                                        ...
                                        return <Icon size={26} className="text-primary group-hover:scale-110 transition-transform" />;
                                    })()}
                                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-16 items-start">
                    ...
                    >
                        <div className="md:sticky md:top-24">
                            <div className="relative rounded-[2.5rem] md:rounded-[3rem] overflow-hidden group mb-8 md:mb-10 border border-white/10 shadow-3xl bg-surface/20 p-2 aspect-square">
                                <OptimizedImage
                                    src={founder.image}
                                    alt={founder.name}
                                    className="rounded-[2rem] md:rounded-[2.5rem] grayscale transition-all duration-700 group-hover:grayscale-0 group-hover:scale-105 object-cover"
                                />
                                <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6 p-4 md:p-6 bg-black/40 backdrop-blur-md rounded-2xl border border-white/10 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-500">
                                    <h3 className="text-base md:text-xl font-bold text-white">{founder.name}</h3>
                                    <p className="text-primary text-[8px] md:text-[10px] font-black uppercase tracking-widest mt-1">{founder.role}</p>
                                </div>
                            </div>
                            <div className="space-y-6 md:space-y-8 flex flex-col items-center lg:items-start text-center lg:text-left">
                                <h3 className="text-xl md:text-3xl font-serif font-bold text-white tracking-tight italic leading-snug">"{founder.quote}"</h3>
                                <p className="text-gray-300 text-base md:text-lg leading-relaxed max-w-lg">{founder.bio}</p>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    <div className="lg:col-span-7 space-y-10 md:space-y-12 mt-12 md:mt-0">
                        ...
                        <div className="grid grid-cols-1 gap-6 md:gap-8">
                            {ecosystem.map((box: any, i: number) => {
                                if (box.highlight) {
                                    return (
                                        <motion.div
                                            key={i}
                                        ...
                                        >
                                            ...
                                            <div className="flex flex-col sm:flex-row gap-6 md:gap-8 items-start h-full">
                                                <div className="w-14 h-14 md:w-16 md:h-16 bg-primary/20 rounded-2xl flex-shrink-0 flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                                                    {(() => {
                                                        const Icon = IconMap[box.icon] || Target;
                                                        return <Icon size={32} />;
                                                    })()}
                                                </div>
                                                <div className="flex flex-col h-full">
                                                    <h3 className="text-xl md:text-2xl font-bold text-white mb-3 md:mb-4">{box.title}</h3>
                                                    <p className="text-gray-300 text-base md:text-lg leading-relaxed mb-6 flex-1">{box.desc}</p>
                                                    <div className="flex flex-wrap gap-2 mt-auto">
                                                        {(box.tags || []).map((tag: string, j: number) => (
                                                            <span key={j} className="px-3 py-1 bg-white/5 rounded-lg text-[10px] font-bold text-gray-300 border border-white/5">{tag}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 items-stretch">
                                {ecosystem.filter((b: any) => !b.highlight).map((box: any, i: number) => (
                                    <motion.div
                                        key={i}
                                        initial={{ opacity: 0, y: 20 }}
                                        whileInView={{ opacity: 1, y: 0 }}
                                        ...
                                        className="h-full bg-surface/20 backdrop-blur-md p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-white/10 group hover:border-white/30 transition-all duration-500 flex flex-col"
                                    >
                                        <div className="w-10 h-10 md:w-12 md:h-12 bg-white/5 rounded-xl flex-shrink-0 flex items-center justify-center mb-4 md:mb-6 group-hover:bg-primary/20 transition-all">
                                            {(() => {
                                                const Icon = IconMap[box.icon] || Award;
                                                return <Icon size={22} className="text-white group-hover:text-primary transition-colors" />;
                                            })()}
                                        </div>
                                        <h3 className="text-lg md:text-xl font-bold text-white mb-3 md:mb-4">{box.title}</h3>
                                        <p className="text-gray-300 text-sm leading-relaxed flex-1">{box.desc}</p>
                                    </motion.div>
                                ))}
                            </div>
                        </div>
            ...
            <section className="container mx-auto px-6 mb-16 md:mb-24 relative z-10">
                ...
                >
                    ...
                    <div className="relative z-10 max-w-4xl mx-auto text-center">
                        <span className="bg-primary/20 text-primary font-black uppercase text-[10px] tracking-[0.2em] px-4 py-2 rounded-full mb-8 md:mb-10 inline-block border border-primary/20">Horizon {visionHorizon}</span>
                        <h2 className="text-3xl md:text-7xl font-serif font-bold text-white mb-8 md:mb-10 tracking-tight leading-tight">
                            {(() => {
                                const lines = visionTitle.split('.');
                                if (lines.length > 1) {
                                    return <>{lines[0]}. <br /> Bâtir <span className="text-primary italic">{lines[1]}</span></>;
                                }
                                // If not splitting exactly as expected, just show it
                                return <span>{visionTitle}</span>;
                            })()}
                        </h2>
                        <p className="text-gray-300 text-base md:text-xl leading-relaxed mb-10 md:mb-16 max-w-2xl mx-auto">{visionContent}</p>
