...
const ContactClient = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.honeypot !== "") {
            ...
            setStatus('success'); // Fake success for the bot
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                setStatus('success');
                setFormData({ name: "", email: "", phone: "", countryCode: "+221", subject: "Consulting Stratégique", message: "", honeypot: "" });
    ...
    return (
        <div className="pt-24 md:pt-40 pb-16 md:pb-20 px-0 md:px-6 min-h-screen bg-transparent relative overflow-hidden">
            ...
            <div className="container mx-auto relative z-10 px-6 md:px-0">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-12 max-w-7xl mx-auto">
                    {/* Form */}
                    <motion.div
                        initial={{ opacity: 0, x: -30 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: 0.4, duration: 0.8 }}
                        className="lg:col-span-7 bg-surface/40 backdrop-blur-xl p-6 md:p-12 rounded-[2.5rem] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.3)] relative group"
                    >
                        ...
                        {status === 'success' ? (
                            ...
                            >
                                <motion.div
                                    ...
                                    className="w-20 h-20 md:w-24 md:h-24 bg-green-500/20 rounded-3xl flex items-center justify-center mb-8 rotate-3 shadow-lg shadow-green-500/10"
                                >
                            ...
                            <div className="relative z-10">
                                <h2 className="text-2xl md:text-3xl font-bold mb-8 md:mb-10 text-white flex items-center gap-4">
                                    Envoyer un message
                                ...
                                <form className="space-y-6 md:space-y-8" onSubmit={handleSubmit}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                                        ...
                                        >
                                            ...
                                            <input
                                                required
                                                type="text"
                                                className="w-full bg-background/30 border border-white/5 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-primary/50 focus:bg-background/60 focus:ring-4 focus:ring-primary/5 transition-all placeholder:text-gray-600 text-sm md:text-base"
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-3">
                                            <select
                                                className="bg-background/20 border border-white/5 rounded-2xl px-4 py-4 text-white focus:outline-none focus:border-primary/50 transition-all text-xs md:text-sm w-28 md:w-32 backdrop-blur-md appearance-none cursor-pointer"
                                            ...
                                            <input
                                                type="tel"
                                                inputMode="tel"
                                                className="flex-1 bg-background/30 border border-white/5 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-primary/50 focus:bg-background/60 focus:ring-4 focus:ring-primary/5 transition-all placeholder:text-gray-600 text-sm md:text-base"
                                    ...
                                    >
                                        ...
                                        <select
                                            className="w-full bg-background/30 border border-white/5 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-primary/50 focus:bg-background/60 focus:ring-4 focus:ring-primary/5 transition-all appearance-none cursor-pointer text-sm md:text-base"
                                    ...
                                    >
                                        ...
                                        <textarea
                                            required
                                            rows={5}
                                            className="w-full bg-background/30 border border-white/5 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-primary/50 focus:bg-background/60 focus:ring-4 focus:ring-primary/5 transition-all resize-none placeholder:text-gray-600 text-sm md:text-base"
                                    ...
                                    <div className="pt-4">
                                        <Button
                                            ...
                                            className="w-full h-14 md:h-16 text-lg md:text-xl font-bold rounded-2xl shadow-[0_10px_30px_rgba(255,215,0,0.2)] hover:shadow-[0_15px_40px_rgba(255,215,0,0.3)] transition-all flex items-center justify-center gap-3 overflow-hidden group"
                                        >
                                            {status === 'loading' ? (
                                                <>
                                    ...
                                    {status === 'error' && (
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            className="bg-red-500/10 border border-red-500/20 p-4 rounded-xl text-red-500 text-xs font-medium text-center"
                                        >
                                ...
                                {[
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            {item.href ? (
                                                <a href={item.href} className="text-white font-bold text-sm md:text-lg leading-tight mb-1 hover:text-primary transition-colors block truncate">
                                                    {item.content}
                                                </a>
                                            ) : (
                                                <p className="text-white font-bold text-sm md:text-lg leading-tight mb-1">{item.content}</p>
                                            )}
                                            {item.sub && <p className="text-gray-500 text-[10px] md:text-xs">{item.sub}</p>}
                                        </div>
                                    </motion.div>
                                ))}
                            </div>
