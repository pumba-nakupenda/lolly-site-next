...
const ServicesClient = ({
    ...
    const handleContactClick = (subject?: string) => {
        ...
        if (isMobile) {
            router.push(`/contact${subjectParam}`);
        } else {
    ...
    const defaultFaqs = [
        { q: "Comment débute une collaboration avec LOLLY ?", a: "Tout commence par une consultation stratégique offerte. Nous analysons vos besoins réels avant de vous proposer une roadmap personnalisée." },
        { q: "Proposez-vous des formations sur mesure ?", a: "Absolument. Nos modules Digital Skills sont adaptés au niveau de vos équipes et aux spécificités de votre secteur d'activité." },
        { q: "Quels sont vos délais habituels ?", a: "Pour un branding complet, comptez 3 à 5 semaines. Pour une production vidéo institutionnelle, entre 2 et 4 semaines selon l'envergure." },
        { q: "Accompagnez-vous vos clients sur le long terme ?", a: "Oui, nous proposons des forfaits d'accompagnement mensuels pour le consulting stratégique et la gestion des réseaux sociaux." }
    ];
    ...
    const standardExpertises = displayExpertises.filter(s => s.id !== highlightExpertise.id);
    const displaySteps = dynamicSteps && dynamicSteps.length > 0 ? dynamicSteps : defaultSteps;
    const displayResults = dynamicResults && dynamicResults.length > 0 ? dynamicResults : defaultResults;
                        ...
                        >
                            ...
                            >
                                ...
                                {(() => {
                                    ...
                                    return <Icon className="text-white relative z-10 group-hover:scale-110 transition-transform" size={32} />;
                                })()}
                            </motion.div>
                ...
                >
                    <div className="bg-surface/20 backdrop-blur-2xl border border-primary/20 rounded-[3rem] p-10 md:p-16 relative overflow-hidden shadow-2xl transition-all duration-300 group-hover:border-primary/50 group-hover:shadow-[0_0_50px_rgba(255,215,0,0.15)]">
                        ...
                        <div className="flex flex-col lg:flex-row items-center gap-12 relative z-10">
                            ...
                            >
                                {(() => {
                                    const Icon = IconMap[highlightExpertise.icon] || MessageSquare;
                                    return <Icon size={56} className="text-primary" />;
                                })()}
                            </motion.div>
                            <div className="text-center lg:text-left">
                                <h3 className="text-4xl md:text-5xl font-serif font-bold text-white mb-4 tracking-tight">{highlightExpertise.title}</h3>
                                <p className="text-gray-300 text-xl mb-10 max-w-3xl leading-relaxed">{highlightExpertise.description}</p>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {(highlightExpertise.items || []).map((item: any, i: number) => (
                ...
                >
                    {standardExpertises.map((service, index) => (
                        <motion.div
                            ...
                            className="min-w-[80vw] snap-center md:min-w-0 bg-surface/30 backdrop-blur-xl rounded-[2.5rem] border border-white/5 overflow-hidden hover:border-primary/20 transition-all duration-500 flex flex-col h-full group relative shadow-xl cursor-pointer"
                            onClick={() => router.push(service.link || '/contact')}
                        >
                            ...
                            <div className="p-6 md:p-10 flex-1 relative z-10">
                                <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center mb-8 border border-white/10 group-hover:bg-primary/20 transition-all group-hover:rotate-12 group-hover:scale-110">
                                    {(() => {
                                        ...
                                        return <Icon size={28} className="text-white group-hover:text-primary transition-colors" />;
                                    })()}
                                </div>
                    ...
                    {displayResults.map((stat: any, i: number) => (
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-gray-300 text-sm leading-relaxed">{stat.description || stat.desc}</p>
                            </div>
                            {/* Inner Glow */}
