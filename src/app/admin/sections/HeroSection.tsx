...
export default function HeroSection() {
  const { items, loading, error, saveError, saving, create, update } = useAdminTable<Hero>("hero");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    let ok: boolean;
    if (hero) ok = !!(await update(hero.id, form));
      ...
      <AdminCard>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-4 pt-2">
            ...
            {saved && <span className="text-green-400 text-sm font-semibold">✓ Sauvegardé !</span>}
          </div>
        </form>
      </AdminCard>
    </div>
